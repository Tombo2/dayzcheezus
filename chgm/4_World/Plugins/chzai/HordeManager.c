class HordeManager : PluginBase
{
	...
	private ref array<ref CHZAIH> m_Hordes;

   void HordeManager()
   {
      if(GetGame().IsClient() || !GetGame().IsMultiplayer()) return;
      MakeDirectory("$profile:CHZAIH");
   ...
   override void OnDestroy()
   {
	...
	void RemoveHorde()
	{
		...
		if(m_Config.GetDespawnTimeLength() == 0)
		{
			...
			foreach(ref CHZAIH horde : m_Hordes)
			{
				ref CHZAIH hordeToBeRemoved = horde;
		...
		ref CHZAIH removedHorde = m_Hordes[0];
   ...
   void AddHorde()
   {
		...
		ref CHZAIH newHorde = new CHZAIH(hordePosition, m_Config.CanDeleteHorde(), m_Config.DeleteOnlyAlive());
